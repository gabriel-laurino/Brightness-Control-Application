...
class SettingsController:
    def __init__(self, parent_window, config_manager, brightness_controller):
        ...
        self.lang_strings = self.config_manager.load_language_strings(self.language_code)
        self.view = SettingsView(parent_window, self)
        self.view.set_controller(self)
        self.brightness_controller = brightness_controller
        self.log_service = LogService()  # Instancia o serviço de log
    ...
    def validate_schedule(self, schedule):
        ...
        for start, end in periods:
            if start not in schedule or end not in schedule:
                self.log_service.log_warning(f"Schedule missing start or end for period: {start}, {end}")
                return False
            start_time = schedule[start]
            end_time = schedule[end]

            if not (0 <= start_time <= 24) or not (0 <= end_time <= 24):
                self.log_service.log_warning(f"Schedule times out of bounds for period: {start} - {end}")
                return False
            ...
            if start_time < end_time:
                ...
                if wrap_around_count > 1:
                    self.log_service.log_warning("Multiple wrap-around periods detected.")
                    return False
                intervals.append((start_time, 24))
                intervals.append((0, end_time))
            else:
                self.log_service.log_warning(f"Start time equals end time for period: {start} - {end}")
                return False
        ...
        for i in range(len(intervals_sorted) - 1):
            ...
            if current_end > next_start:
                self.log_service.log_warning(f"Overlap detected between intervals: {intervals_sorted[i]} and {intervals_sorted[i + 1]}")
                return False
    ...
    def convert_to_12_hour(self, hour_24):
        self.log_service.log_debug(f"Converting {hour_24} from 24-hour to 12-hour format.")
